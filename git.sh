...
#spf_depend gettext  when runtime &&
spf_depend libressl degree optional &&
spf_args configure '--without-python' &&
spf_args configure '--without-tcltk' &&
spf_args configure "--with-curl=${spf_path_conf?}/curl/prefix"
spf_args configure "--with-openssl=${spf_path_conf?}/libressl/prefix" &&
case ${spf_version?} in
  1.5.*|1.6.[012345].*) :;;
  *)
    spf_data dir etc &&
...
spf_template_gnu_do_make() {
  msgfmt_path=$(which msgfmt) || ( echo "Need a working msgfmt from gettext for git to install" && exit 1 )  &&
  perl_path=$(which perl) || ( echo "Need a working perl for git to install" && exit 1 )  &&
  echo "Using PERL_PATH=${perl_path?}" &&
  PERL_PATH=${perl_path?} make 
} &&
spf_template_gnu_do_install() {
  perl_path=$(which perl) || ( echo "Need a working perl for git to install" && exit 1 )  &&
  echo "Using PERL_PATH=${perl_path?}" &&
  PERL_PATH=${perl_path?} make install
} &&
